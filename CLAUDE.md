...
Comments in this codebase are often written in German. When editing code that contains German comments, translate them to English. Always write new comments in English.

## Changelog

New functionality must be recorded in `CHANGES.md` (repository root). Add a short entry describing what was added/changed when you implement it — don't wait to be asked.
...
- **Project file:** `Pdp11gui/pdp11GUI.lpi` (open this in Lazarus to build)
- **Main program:** `Pdp11gui/pdp11GUI.dpr`
- **Unit output:** `Pdp11gui/lib/$(TargetCPU)-$(TargetOS)/`
- **Build:** `./build.sh` from the repository root, or open `Pdp11gui/pdp11GUI.lpi` in Lazarus and use Build/Compile menu
- **External tools at runtime:** `Pdp11gui/macro11.bat` (invokes MACRO11.exe assembler), `Pdp11gui/m4.bat` (M4 preprocessor for driver templates)

## Linux/Qt5 HiDPI Notes

The app must be built with the Qt5 widgetset (`lazbuild --ws=qt5 pdp11GUI.lpi`, or just `./build.sh`) — GTK2 (lazbuild's default) has no HiDPI support at all, and GTK3 crashes on startup. Run via `Pdp11gui/run.sh`, which computes `QT_SCALE_FACTOR` from the desktop's `Xft.dpi` and also sets `QT_FONT_DPI=96`.
...
- Concrete console implementations follow the naming pattern `ConsolePDP11*U.pas`:
  ...
  - `ConsolePDP11SimHU` — SimH simulator over Telnet (both the "Modified SimH over telnet" and "SimH direct" connection types — see `CHANGES.md`)
...
**External modules (relative to `Pdp11gui/`)**
...
`Pdp11gui/driver/` contains MACRO-11 assembly source for device drivers that get compiled and loaded into PDP-11 memory at runtime:
